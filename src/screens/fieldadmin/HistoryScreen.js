...
const HistoryScreen = ({ navigation }) => {
  ...
  const renderAssessments = () => {
    if (loading) return <ActivityIndicator color={theme.colors.primary.main} style={{ marginTop: 20 }} />;
  ...
  const renderTrucks = () => {
    ...
    if (trucks.length === 0) {
      return (
        <Card variant={theme.isDarkMode ? 'glass' : 'default'} style={styles.itemCard}>
          <Text style={[styles.itemMeta, { color: theme.colors.text.secondary }]}>
            No truck runs in the last 7 days.
          </Text>
        </Card>
      );
    }
  ...
  const renderDrivers = () => {
    ...
    if (drivers.length === 0) {
      return (
        <Card variant={theme.isDarkMode ? 'glass' : 'default'} style={styles.itemCard}>
          <Text style={[styles.itemMeta, { color: theme.colors.text.secondary }]}>
            No driver activity in the last 7 days.
          </Text>
        </Card>
      );
    }
